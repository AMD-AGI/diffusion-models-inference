...
def expected_result_paths(config_dir: Path) -> list[Path]:
    expected = []
    for config_path in sorted(config_dir.glob("*.yaml")):
        ...
        for benchmark in config.get("benchmarks", []):
            if benchmark.get("type") != "xdit":
                continue
            output_directory = Path(benchmark.get("args", {}).get("output_directory", ""))
            if not output_directory.is_absolute():
                raise ValueError(f"Invalid xDiT output_directory in {config_path}: {output_directory}")
            expected.append(output_directory / "timings.json")
    return expected
...
def main() -> None:
    ...
    expected = expected_result_paths(args.config_dir)
    if not expected:
