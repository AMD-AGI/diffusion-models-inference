...
from ..benchmark_sources import _ALT_DELIMITER, get_benchmark_sources, parse_filter
...
def _compute_psnr_images(ref_img: Path, distorted_img: Path, logger: LoggingConsole) -> float:
    """Compute SSIM for a given reference and distorted image."""
    logger.trace(f"Computing SSIM for image '{ref_img}' and '{distorted_img}'")
    try:
        reference = imread(ref_img)
        distorted = imread(distorted_img)
        if reference.shape != distorted.shape:
            logger.error(
                f"Cannot compute SSIM for images '{ref_img}' and '{distorted_img}': "
                f"image shapes differ ({reference.shape} != {distorted.shape})"
            )
            return float("nan")

        # Use channel_axis=2 for RGB/BGR images to evaluate channels together
        score, diff = ssim(reference, distorted, channel_axis=2, full=True)

        return float(score)
    except Exception as error:  # noqa: BLE001 - metric failures are reported as NaN
        logger.error(
            f"Failed to compute SSIM for images '{ref_img}' and '{distorted_img}': {error}"
        )
        return float("nan")
...
def metric_psnr(logger: LoggingConsole, results_dir: Path, args: list[str] | None = None) -> int:
    ...
    "as reference."""
    logger.critical("The tool is not implemented. PSNR is unlikely to ever be used for this task.")
    return 1
    ...
    for file,res in coll.items():
        logger.trace(f"{file}:")
        for r in res:
            logger.trace(f"  {r['psnr']:07.5f} : {r['comparison_pfx']}")
