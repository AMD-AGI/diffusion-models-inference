...
def image_label(image: str) -> str:
    """Condense a Docker image reference into a run-name fragment.

    ``ghcr.io/org/xdit:v26.06`` becomes ``xdit_v26.06``. Registry host, port and
    namespace are dropped; digest references keep only the short digest.
    """
    ref = image.rsplit("/", 1)[-1]
    name, digest_sep, digest = ref.partition("@")
    if digest_sep:
        return f"{name}_{digest.rsplit(':', 1)[-1][:12]}"
    name, tag_sep, tag = ref.partition(":")
    return f"{name}_{tag}" if tag_sep and tag else name
...
def convert_entry(
    ...
    Args:
        ...
        output_root: Directory results are written to, as seen by the driver.
        mlflow_uri: MLflow tracking URI; defaults to a file store under output_root.
        experiment_prefix: Workspace folder holding the experiment. Must already
            exist on Databricks; empty means the workspace root.
        arch_label: GPU architecture shown in the run name, e.g. ``gfx950``.
    ...
    args["output_directory"] = f"{output_root}/{sanitize_name(entry.get('name', 'xdit'))}"

    server_args = dict(DEFAULT_SERVER_ARGS)
    server_args["image"] = image

    # inference-testing groups runs by model, so configs sharing a model become
    # sibling runs in one experiment. Mirrors csv_to_mlflow/lucid_csv_to_mlflow.
    prefix = experiment_prefix.rstrip("/")
    if prefix and not prefix.startswith("/"):
        prefix = f"/{prefix}"
    experiment_name = f"{prefix}/{entry['model'].replace('/', '--')}"

    # Runs are siblings under one model, so the name has to carry what separates
    # them: which image was benchmarked, and on which GPU.
    run_name = " | ".join(part for part in (image_label(image), entry.get("name", "xdit"), arch_label) if part)

    config: dict[str, Any] = {
        "reporting": {
            "type": "mlflow",
            "args": {
                "uri": mlflow_uri or f"file://{output_root}/mlruns",
...
def convert_file(
    ...
    for entry in entries:
        ...
        converted = convert_entry(entry, image, output_root, mlflow_uri, experiment_prefix, arch_label)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-root",
        default="/outputs",
        help="Directory results are written to, as seen by the driver (default: /outputs)",
    )
    parser.add_argument(
        "--mlflow-uri",
        default="",
        help="MLflow tracking URI, e.g. 'databricks' (default: a file store under --output-root)",
    )
    parser.add_argument(
        "--experiment-prefix",
        default="",
        help="Prepended to experiment names; Databricks needs an absolute path, e.g. /Shared/xdit",
    )
    parser.add_argument(
        "--arch-label",
        default="",
        help="GPU architecture shown in the MLflow run name, e.g. gfx950",
    )
