...
def render_report_md(
    ...
    lines = [
        ...
        f"- **Primary A/B comparisons** (in system DB): {comparison.get('primary_ab_count', 0)}",
        f"- **Improvements** (exhaustive faster): {counts.get('improvement', 0)}",
        f"- **No change**: {counts.get('no_change', 0)}",
        f"- **Regressions** (solver changed + slower): {counts.get('regression', 0)}",
        f"- **System DB misses**: {counts.get('system_db_miss', 0)}",
        ...
        "- **Arm A**: production-like path (`MIOPEN_FIND_ENFORCE=3`, default find mode, system DB enabled, shared user DB)",
        "- **Arm A measurement**: inline timing from find/tune driver invocation",
        "- **Arm B tuning**: exhaustive override (`MIOPEN_FIND_ENFORCE=3`, `MIOPEN_SYSTEM_DB_PATH=$MIOPEN_USER_DB_PATH`)",
        "- **Arm B benchmark**: `MIOPEN_FIND_ENFORCE=1` with merged user DB",
        ...
        "",
        "## Improvements (system DB suboptimal)",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- `{output_dir / 'arm_a/user_db/'}` (Arm A user DB — `.udb.txt` / `.ufdb.txt`)",
